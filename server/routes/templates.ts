 ...
 * @desc List all versions for a template
 * @access Private
 */
router.get(
  "/:id/versions",
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const versions = await prisma.signatureTemplateVersion.findMany({
    ...
    successResponse(res, versions, "Template versions retrieved successfully");
  })
);

/**
 * @route POST /api/templates/:id/rollback
 * @desc Restore a previous version (by version number or version ID)
 * @access Private
 * @body {number} versionId or version
 */
router.post(
  "/:id/rollback",
  authenticateToken,
  validateTenantAccess,
  validate({ params: commonSchemas.id, body: Joi.object({ versionId: Joi.number().optional(), version: Joi.number().optional() }) }),
  asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const { versionId, version } = req.body;
    if (!versionId && !version) {
      throw new ApiError(HTTP_STATUS.BAD_REQUEST, "versionId or version is required");
    }
    const where: any = { template_id: Number(id) };
    if (versionId) where.id = versionId;
    if (version) where.version = version;
    const versionRecord = await prisma.signatureTemplateVersion.findFirst({ where });
    if (!versionRecord) {
      throw new ApiError(HTTP_STATUS.NOT_FOUND, "Version not found");
    }
    // Update template to match version
    const updatedTemplate = await prisma.signatureTemplate.update({
      where: { id: Number(id) },
      data: {
        name: versionRecord.name,
        html_content: versionRecord.html_content,
      },
    });
    successResponse(res, updatedTemplate, "Template rolled back to selected version");
  })
);
